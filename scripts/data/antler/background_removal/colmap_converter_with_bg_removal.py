...
def get_main_cluster_bounds(points3d_path, eps=0.15, min_samples=20):
    ...
    # Raised from 60 to 85. Tine tips are thin and hard to match, so they have higher reconstruction error. The old 60th percentile cutoff was discarding them before DBSCAN even ran.
    error_threshold = np.percentile(errors, 85)
...
def create_antler_mask_from_colmap(
        image_shape,
        projected_points,
        point_radius=45,
        expansion_ratio=1.0):
    ...
    # Fill the convex hull of all projected points. Tines radiate outward from the main beam, so their tips land on the hull boundary.
    # Without this, sparse tine tip circles were isolated islands that the subsequent AND with rembg's mask wiped out.
    pts_array = np.array(projected_points, dtype=np.int32)
    if len(pts_array) >= 3:
        hull = cv2.convexHull(pts_array)
        cv2.fillPoly(mask, [hull], 255)

    if expansion_ratio > 1.0:
        k = int(25 * expansion_ratio)
        k = max(3, k | 1)
...
def process_single_antler(antler_id, working_dir, view_dir, use_colmap_guidance=True, save_debug=False):
    ...
    if use_colmap_guidance and os.path.exists(points3d_txt):
        ...
        # min_samples reduced from 15 to 5 so sparse tine neighborhoods aren't dropped as DBSCAN noise.
        main_object_bbox = get_main_cluster_bounds(points3d_txt, eps=0.12, min_samples=5)
    ...
    print("\nInitializing background removal (birefnet-general model)")
    print("(First run downloads model - may take 5-10 minutes)")
    import sys
    sys.stdout.flush()
    ...
    try:
        ...
        print("Using GPU-accelerated background removal (BiRefNet)")
    except:
        rembg_session = rembg.new_session(
            model_name='birefnet-general',
            providers=['CPUExecutionProvider']
        )
        print("✓ Using CPU-based background removal (BiRefNet)")
    ...
    for i, name in enumerate(names):
        ...
        try:
            print(f"Step 1: Initial segmentation with rembg (pass 1)")
...
if __name__ == "__main__":
    ...
    print(f"{'='*70}")
    print(f"ANTLER DATA PREPARATION WITH OUTLIER DETECTION")
    print(f"{'='*70}")
